    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p.absolute)
            {
            	...
            	var edWidth = g.selectBox.width();
            	var bodyWidth = $('body').width();
            	var pleft = owrapper.offset().left;
            	var oleft=0;
            	if(edWidth+pleft>bodyWidth-20){
            		oleft = edWidth+pleft-bodyWidth+20;
            	}
            	
            	var gtop = owrapper.offset().top + 1 + owrapper.outerHeight();
            	var gheight = g.selectBox.height();
            	var  sheight = $(window).height();
            	var otop = owrapper.offset().top;
            	//alert(sheight+":"+otop);
            	if(sheight<otop+gheight){
            		gtop  = owrapper.offset().top-gheight-1;
            	}
            	g.selectBox.css({ left: owrapper.offset().left-oleft, top: gtop });
                // g.selectBox.css({ left: owrapper.offset().left, top: owrapper.offset().top + 1 + owrapper.outerHeight() });
            }
            else
            {
