...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        cssClass: null,                    //类名
        root: 'content',                       //数据源字段名
        record: 'totalElements',                     //数据源记录数字段名
        pageParmName: 'page.page',               //页索引参数名，(提交给服务器)
        pagesizeParmName: 'page.size',        //页记录数参数名，(提交给服务器)
        sortnameParmName: 'page.sort',        //页排序列名(提交给服务器)
        sortorderParmName: 'page.sort.dir',      //页排序方向(提交给服务器)
        ...
        showGroupAndTotalSum: false,		   //分组 - 同时显示分组合计和所有总计
    ...
    $.ligerDefaults.Grid.formatters['currency'] = function (value, column)
    {
        var parseToNumber = function(value){
        	var num = value ? value.toString().replace(/[^0-9.-]/g,'') : '';
    ...
    $.ligerDefaults.Grid.editors['date'] =
    {
        create: function (container, editParm)
        {
            ...
            input.val(editParm.value);		//added by yanxiaoguang
    ...
    $.ligerDefaults.Grid.editors['string'] =
     $.ligerDefaults.Grid.editors['text'] = {
         create: function (container, editParm)
         {
             ...
             container.append(input);
             var options = editParm.column.editor.p || {};
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function ()
        {
            ...
            if (p.tree)//启用分页模式           xzhk
            {
                ...
                p.tree.isParent = p.tree.isParent || function (rowData)
                {
                    ...
                    var exist = false;
                    for(str in rowData){
                    	if(str == p.tree.childrenName){
                    		if(rowData[str] != null && rowData[str] != '')exist = true;
                    	}
                    }
        ...
        loadData: function (loadDataParm)
        {
            ...
            if (p.dataAction == "server")
            {
                ...
                {	
	                ...
	                {
	                	 param.push({ name: p.pageParmName, value:1 });
	                }else{
        ...
        {
            var g = this, p = this.options;
            param.push({name:'randomtime', value:new Date().getTime()});
            //alert(0);
            var ajaxOptions = {
                type: p.method,
                url: p.url,
        ...
        */
        ...
        _initColumns: function ()
        {
            ...
            {
            	/*for(var i=0;i<props.length;i++){//fk20131109
                    for(var j=0;j<column.columns.length;j++){
                    	if(props[i]==column.columns[j]){
        ...
        _fillGridBody: function (data, frozen)
        {
            ...
            if (g.enabledGroup()) //启用分组模式
            {
                ...
                $(groupsdata).each(function (i, item)
                {
                    ...
                    if(p.withinGroupRender){
                    	gridhtmlarr.push(p.withinGroupRender(item, frozen, g, i, groupsdata.length));
                    }else if(p.showTotalOnly){
                    	//Nothing to do
                    }else{
            ...
            //分组时不需要            
            if (!g.enabledGroup() || p.showGroupAndTotalSum) //modified by yanxiaoguang - add attribute to allow showing total sum for group grid
            {
                //创建汇总行
        ...
        _getCellContent: function (rowdata, column)
        {
            ...
            var value = column.name ? this._getFieldValue(rowdata, column.name) : null;
            var g = this, p = this.options;
            var content = "";
            if (column.render)
            {
        ...
        _getFieldValue: function (data, fieldname)
        {
                ...
                g.bind('sysEndEdit', function ()
                {
                    ...
                    if (newValue != currentdata)
                    {
                        ...
                        if (g._checkEditAndUpdateCell(editParm))
                        {
                            ...
                            //  xzhk
                            if (column.editor.onMakeSaveData) column.editor.onMakeSaveData(rowdata, column, rowcell, newValue);
                        }
                    }
                });
            }
        ...
        _getTotalCellContent: function (column, data)
        {
            ...
            if (column.totalSummary)
            {
                ...
                var max = parseFloat(g._getFieldValue(data[0],column.name));  //yanxiaoguang : use _getFieldValue() to support getting nested property
                var min = parseFloat(g._getFieldValue(data[0],column.name));
                for (var i = 0; i < data.length; i++)
                {
                    count += 1;
                    var value = parseFloat(g._getFieldValue(data[i],column.name));
                    if (!value) continue;
                    sum += value;
                        ...
                        if (g.rowDropIn != -1){
                            ...
                            if (p.tree){
                                if (g.rowDropInParent){
                                    parentData = g.getRow(g.rowDropIn);
                                }else{
                                    neardata = g.getRow(g.rowDropIn);
                                    parentData = g.getParent(neardata);
                                }
                                
                                /************************  判别是否 是将父节点移动到子结点中  *******************************/
                                var locationParent = parentData;
                                var isToChildren = false;
                                while(locationParent?true:false){
                                	if(locationParent.praentId == rows[0].id){
                                		isToChildren = true;
                                		break;
                                	}
                                	locationParent = g.getParent(locationParent);
                                }
                                if(isToChildren){
                                	alert("不能将父节点移动到子节点中！");
                                	g.rowDropIn = -1;
                                ...
                                g.appendRange(rows, parentData, neardata, g.rowDropDir != "bottom");
        ...
        _onClick: function (e)
        {
            ...
            else if (src.groupbtn)
            {
                ...
                var withinGroupOpening = true;
                while (true)
                {
                	if(currentRow.hasClass("l-grid-grouprow")){
                		if(grouprow.hasClass("l-grid-row")) break;
                		hasWithinGroup = true;
                		if($('.l-grid-group-togglebtn',currentRow).hasClass('l-grid-group-togglebtn-close')){
                			withinGroupOpening = false;
                		}else{
                			withinGroupOpening = true;
                		}
                	}
                    ...
                    if (opening)
                    {
                    	if(hasWithinGroup && !currentRow.hasClass("l-grid-grouprow") && !withinGroupOpening){
                    		//
                    	}else{
                            currentRow.show();
        ...
        _onResize: function ()
        {
           ...
           // alert( g.gridview.height()+"###########"+g.gridview1.height()+"#########"+g.gridview2.height());
            g.trigger('SysGridHeightChanged');
            
           
           // alert( hangshu+"###########"+hangshu2);
           
        },
        _reSetRowsHeight:function(){// fk20131110
        	 var g = this, p = this.options;
        	 var fda = g.f.gridbody.find(".l-grid-row");
             var rda =  g.gridbody.find(".l-grid-row");
             var hangshu =  fda.length;
             var hangshu2 = rda.length;
           // alert( hangshu+"########han###"+hangshu2);
             if(hangshu==hangshu2&&hangshu!=0){
             	// alert( hangshu+"########han###"+hangshu2);
            	 g.f.gridbody.find(".l-grid-row").removeAttr('style');
            	 g.gridbody.find(".l-grid-row").removeAttr('style');
            	//  $(".font_detail").height(22);
             	for(var i=0;i<hangshu;i++){
             		if($(fda[i]).height()>$(rda[i]).height()){
             			$(rda[i]).height($(fda[i]).height());
             			//$(fda[i]).find(".font_detail").css("height",$(fda[i]).height());
             		}else{
             			$(fda[i]).height($(rda[i]).height());
             			//$(fda[i]).find(".font_detail").css("height",$(rda[i]).height());
             		}
             	}
             }
             
            // $(".font_detail").each(function(){
            //	 $(this).height($(this).parents("td").eq(0).height());
            // });
        }
    });
