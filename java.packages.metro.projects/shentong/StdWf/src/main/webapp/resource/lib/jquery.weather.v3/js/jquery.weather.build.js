...
;(function(_doc){
	var optionDefaults = {
			...
			city		: '南昌', //可以指定城市。支持 encodeURI 编码 ， 和 中文汉字,
	...
	function creatWeather( optionDefaults, _parameter){		
		...
		;jQuery(function($){
			...
			//	console.log(" option.area1:"+ option.area);
			//	console.log(" option.city:"+ encodeURI(encodeURI(option.city)));
			//	console.log(" option.city:"+ option.city);
			//	console.log(" option.area:"+ option.area);
				//如果不存在 父级容器，就创建
				...
				include(['http://php.weather.sina.com.cn/iframe/index/w_cl.php?code=js&day=0&city='+ cityName +'&dfc=1&charset='+ charset], function(){
					...
					for( city in window.SWther.w ){					
						...
						weatherData = {
							...
							day_wind:dataInfo.d1+""+dataInfo.p1+"级",
							night_wind: dataInfo.d2+""+dataInfo.p2+"级"
						};
					}
