    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p.absolute)
            {
            	...
            	var gtop = g.text.offset().top + 1 + g.text.outerHeight();
            	var gheight = g.dateeditor.height();
            	var  sheight = $(window).height();
            	var otop = g.text.offset().top;
            	//alert(sheight+":"+otop);
            	if(sheight<otop+gheight){
            		gtop  = g.text.offset().top-gheight-1;
            	}
            	
                g.dateeditor.css({ left: g.text.offset().left-oleft, top: gtop });
            }
            else
            {
