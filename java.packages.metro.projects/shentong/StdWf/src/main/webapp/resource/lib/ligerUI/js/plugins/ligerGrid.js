...
(function ($)
{
    ...
    $.ligerDefaults.Grid.formatters['currency'] = function (value, column)
    {
        ...
        var formatWan = function(value) {	//按万元显示，保留4位小数
            var num = parseToNumber(value);
            num = (num ? num/10000 : num);	//转成万元
            sign = (num == (num = Math.abs(num)));
            ...
            for (var i = 0; i < Math.floor((num.length-(1+i))/3); i++)
            num = num.substring(0,num.length-(4*i+3))+','+ num.substring(num.length-(4*i+3));
            return (((sign)?'':'-') + num + (cents?(cents+'').substring(1):''));
        };        
        return (column&&column.unit=='w')? formatWan(value) : formatCurrency(value);
