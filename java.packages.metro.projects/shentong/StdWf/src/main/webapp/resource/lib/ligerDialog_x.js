...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
    	colseCallBackFunction: null, // xzhk 给dialog 附加 关闭时回调函数。
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        {
            var g = this, p = this.options;
            if(g.dialog.colseCallBackFunction != null) g.dialog.colseCallBackFunction();
    ...
    $.ligerDialog.open = function (p, colseCallBack)
    {
    	var g = $.ligerDialog(p);
    	if(arguments.length == 1){
    		g.dialog.colseCallBackFunction = null;  // xzhk 添加关闭时调函数
		}else{
			g.dialog.colseCallBackFunction = colseCallBack;  // xzhk 添加关闭时调函数
		}
