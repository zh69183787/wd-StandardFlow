...
<head>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            gridManager = $("#maingrid").ligerGrid({
    			columns: [
    		    			...
    		    			{ display: '立项编号', width: 140, name: 'projectNo' },
    		    			...
    		    			{ display: '操作', name: 'ops',isSort: false, width: 200, render: function(row){
								...
								str += "<a href=javascript:openFlow('"+flowgroup+"','FLOW_STD_APPLY_${sessionScope.SEC_CUR_DEPT.groupcode}','"+id+"','"+tit+"','searchbtn') >"+name+"</a>&nbsp;&nbsp;";
								
								if(state == '2'){
									if(row.stdReview && row.stdReview.flowGroup){
										str += "<a href=javascript:openFlow('"+row.stdReview.flowGroup+"','FLOW_STD_REVIEW_${sessionScope.SEC_CUR_DEPT.groupcode}','"+row.stdReview.id+"','"+tit+"','searchbtn') >查看送审稿</a>&nbsp;&nbsp;";;	
									}else{
										str += "<a href='javascript:addStdReview("+id+")'>填写送审稿</a>&nbsp;&nbsp;";										
									}
								}
    		    				return str;
    		    			}
        ...
        function viewStd(id){
           	parent.f_addTab('std'+id,'标准制修订立项表详情','../stdApply/add?editable=false&id='+id);
        }
        ...
        { 
            if(item.id)
            {
                ...
                {
                    case "add":
                    	winDialog = $.ligerDialog.open({ title: '新增标准化工作', height: 420, url: '../stdApply/add?editable=true', 
                    ...
                    case "modify":
                    	...
                    	winDialog = $.ligerDialog.open({ title: '编辑个人备忘', height: 420, url: '../stdApply/add?editable=true&id='+rowsdata[0].id+'&ts='+new Date().getTime(), 
                     	width: 720, showMax: true, isResize: true, slide: false }); 
                     	winDialog.max();
                        return;
                    case "view":
        ...
        function addStdReview(saId){
        	winDialog = $.ligerDialog.open({ title: '新增送审表', height: 420, url: '../stdReview/add?editable=true&saId='+saId, 
           		width: 720, showMax: true, isResize: true, slide: false }); 
           	winDialog.max();
            return;
        }
...
<form>
	<table cellpadding="0" cellspacing="0" border="0" class="l-table-edit" width="100%">
		<tr style="height:25px;">
			...
			<td align="right" class="l-table-edit-td">立项编号：</td>
			<td align="left" class="l-table-edit-td">
				<input type="text" style="width:90%" name="search_projectNo_like"/>
			</td>										
