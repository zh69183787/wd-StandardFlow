...
<head>
    ...
    <script type="text/javascript">
            ...
            gridManager = $("#maingrid").ligerGrid({
    			columns: [
    		    			...
    		    			{ display: '操作', name: 'ops',isSort: false, width: 200, render: function(row){
								...
								str += "<a href=javascript:openFlow('"+flowgroup+"','FLOW_STD_APPROVE_${sessionScope.SEC_CUR_DEPT.groupcode}','"+id+"','"+tit+"','searchbtn') >"+name+"</a>&nbsp;&nbsp;";								
    		    				return str;
    		    			}
    		    			}], 
    			rownumbers : true,
        ...
        function viewStd(id){
           	parent.f_addTab('std'+id,'标准制修订立项表详情','../stdApprove/add?editable=false&id='+id);
        }
        ...
        { 
            if(item.id)
            {
                ...
                {
                    case "add":
                    	winDialog = $.ligerDialog.open({ title: '新增报批稿', height: 420, url: '../stdApprove/add?editable=true', 
                    ...
                    case "modify":
                    	...
                    	winDialog = $.ligerDialog.open({ title: '编辑报批稿', height: 420, url: '../stdApprove/add?editable=true&id='+rowsdata[0].id+'&ts='+new Date().getTime(), 
                     	width: 720, showMax: true, isResize: true, slide: false }); 
                     	winDialog.max();
                        return;
                    case "view":
