...
<script type="text/javascript">
    $(function (){
    	...
    	$('#stdFunId').change(onFunChange).val('${stdReview.stdFunId}').trigger('change');
    	$('#stdMajorTypeId').change(onMajorChange);
    	$('#stdRemarkItemId').val('${stdReview.stdRemarkItemId}');
    ...
    function saveF(){
	   	 var options = {
	   		...
	   		success:function(obj){
  						if(!obj.flowGroup){
		   					parent.startNewFlow(obj.id,'FLOW_STD_REVIEW_${sessionScope.SEC_CUR_DEPT.groupcode}',obj.stdName,false);
 		...
 		$.ajax({
			...
			success:function(data){
				if(data){
					...
					majorSel.val('${stdReview.stdMajorTypeId}').trigger('change');
				}
			}
		});
	}
			...
			success:function(data){
				if(data){
					...
					if(l > 0){
						...
						minorSel.val('${stdReview.stdMinorTypeId}');
					}else{
            	...
            	<div class="w_bg">
                	<div class="Bottom">
                    	<div class="Top">
                            ...
                            <div class="mb10">
                            <form id="form">
                            	<input type="hidden" name="stdReview.id" id="id" value="${stdReview.id}"/>
                            	<input type="hidden" name="saId" id="saId" value="${param.saId}"/>
                            	<input type="hidden" id="attachGroup" name="stdReview.attachGroup" value="${stdReview.attachGroup}"/>
                                  ...
                                  <tr>
                                    ...
                                    <td><input type="text" class="input_large" style="background: #eeeeee;" name="stdReview.stdName" value="${stdReview.stdName}" id="stdName" maxlength="200" readonly="readonly"/></td>
                                    <td class="lableTd t_r">标准编号</td>
                                    <td><input name="stdReview.stdNo" type="text" class="input_large" id="stdNo" value="${stdReview.stdNo}" maxlength="200"/></td>                                    
                                  </tr>
                                  <tr>
                                    <td class="lableTd t_r">标准化员</td>
                                    <td><input name="cUserName" type="text" class="input_large" id="cUserName" value="${empty stdReview.cUser ? sessionScope.SEC_CUR_ACCOUNT.name : stdReview.cUser.name}" disabled/> 
                                     </td>
                                    <td class="lableTd t_r">申报部门</td>
                                    <td><input name="applyDeptName" type="text" class="input_large" id="applyDeptName" value="${empty stdReview.applyDept ? sessionScope.SEC_CUR_DEPT.name : stdReview.applyDept.name}" disabled/>
									</td>
                                  </tr>
                                  <tr>
                                    <td class="lableTd t_r">申报时间</td>
                                    <td><input name="stdReview.applyDate" type="date" class="input_large l-date" id="applyDate" value="${stdReview.applyDate}"/></td>
                                    <td class="lableTd t_r">立项编号</td>
                                    <td>
                                     	<input name="stdReview.projectNo" type="text" class="input_large" style="background: #eeeeee;" id="projectNo" value="${stdReview.projectNo}" readonly="readonly"/>
									</td>
                                  </tr>
                                  <tr>
                                    <td class="lableTd t_r">标准功能</td>
                                    <td>
										<select id="stdFunId" name="stdReview.stdFunId" class="input_large">
                                    ...
                                    <td>
                                    	<select id="stdMajorTypeId" name="stdReview.stdMajorTypeId" class="input_large" >
										</select>
									</td>
                                  </tr>         
                                  <tr>
                                    ...
                                    <td>
										<select id="stdMinorTypeId" name="stdReview.stdMinorTypeId" class="input_large" disabled>
										</select> 
									</td>
                                    <td class="lableTd t_r">标准备注项</td>
                                    <td>
                                    	<select name="stdReview.stdRemarkItemId" id="stdRemarkItemId" class="input_large" >
                           	  ...
                           	  <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_2">
                            	  <thead>
                          	      ...
                          	      <tr>
                       	          ...
                       	          <tr>
									<td>
										<iframe id="attachGroupIframe" height="35" frameborder="0" autoheight="true" width="80%" scrolling="no" src="../attachment/findByGroup?group=${stdReview.attachGroup}&dest_code=attachGroup&operation=modify&refresh=1&showHistory=1&origNameUsed=true&version=${version}&localFileDir=${fn:replace(stdReview.projectNo,'-','\\')}" ></iframe>
									</td>
								  </tr>                                  
                          	      <tr>
                            	  ...
                            	  <tr>
                            	    <td><textarea id="remark" rows="7" name="stdReview.remark">${stdReview.remark}</textarea></td>
                           	      </tr>                           	      
                       	      </table>
                       	      </form>
                          </div>
